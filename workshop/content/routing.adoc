...
behavior of your application's traffic without requiring code changes. This
change in behavior is handled declaratively through the use of the Service
...
existing example and the visualization present in the Kiali Console.
...
visualization will be used to highlight the change in behavior as we work
through this module.
...
invocation. The default behavior is to distribute requests to each endpoint
...
In this example we will modify the default behavior for distributing
...
[source,bash,role="execute-1"]
----
oc apply -n %username%-tutorial -f /opt/app-root/workshop/content/src/istiofiles/destination-rule.yml
----
...
[source,bash,role="execute-1"]
----
oc apply -n %username%-tutorial -f /opt/app-root/workshop/content/src/istiofiles/virtual-service-80-20.yml
----
...
behavior for distributing requests between the endpoints so we could send
traffic to particular endpoints based on weighting. In this example we will
modify the behavior to be more selective, using characteristics of the
...
not seeing replies from the v2 or v3 endpoints. This is the behavior for all
...
behavior was managed without any modifications to the application's code and
...
more complex routing behavior. This module is designed to provide only a
